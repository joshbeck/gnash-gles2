...
Gtkgles2Glue::prepDrawingArea(GtkWidget *drawing_area)
{
    ...
    if (_device->getType() == renderer::GnashDevice::EGL) {
        ...
        egl->bindClient(renderer::GnashDevice::OPENGLES2);
    }
...
Gtkgles2Glue::createRenderHandler()
{
    ...
    _renderer.reset(reinterpret_cast<renderer::gles2::Renderer_gles2 *>
                    (renderer::gles2::create_handler(0)));
    if (!_renderer) {
        boost::format fmt = boost::format(
            _("Could not create OPENGLES2 renderer"));
